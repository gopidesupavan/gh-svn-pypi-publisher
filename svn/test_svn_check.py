import pytest
from svn.svn_check import check_with_regex, check_files_with_identifiers, unknown_file_extensions, unknown_files


@pytest.mark.parametrize(
"file, pattern, check_type, expected",
    [
        pytest.param("apache-airflow-2.10.3-source.tar.gz",".*(tar.gz)$","extension", True, id="valid_extension"),
        pytest.param("apache-airflow-2.10.3-source.tar12.gz",".*(tar.gz)$","extension", None, id="invalid_extension"),
        pytest.param("apache_airflow-2.10.3-source.tar.gz",".*(apache_airflow.*)$","package_name", True, id="valid_package_name"),
        pytest.param("apache-airflow-2.10.3-source.tar.gz",".*(apache-airflow.*)$","package_name", True, id="valid_package_name"),
        pytest.param("apacheairflow-2.10.3-source.tar.gz",".*(apache-airflow.*)$","package_name", None, id="invalid_valid_package_name"),
])
...
def test_check_files_with_identifiers_for_extension():
    ...
    identifiers = [{"type": "regex", "pattern": ".*(py3-none-any.whl|tar.gz.sha512|tar.gz.asc|tar.gz|py3-none-any.whl.asc|py3-none-any.whl.sha512)$"}]
...
def test_check_files_with_identifiers_for_invalid_extension():
    ...
    check_type = "extension"
    check_files_with_identifiers(identifiers, all_files, check_type)
    assert unknown_file_extensions == ["apache-airflow-2.10.3-py3-none-any.whl.asc123", "apache-airflow-2.10.3.tar.jpeg"]
...
def test_check_files_with_multiple_identifiers_for_package_name():
    ...
    identifiers = [{"type": "regex", "pattern": ".*(apache-airflow.*)$"}, {"type": "regex", "pattern": ".*(apache_airflow.*)$"}]
    check_type = "package_name"
    check_files_with_identifiers(identifiers, all_files, check_type)
    assert unknown_files == []
...
def test_check_files_with_multiple_identifiers_for_invalid_package_name():
    ...
    check_type = "package_name"
    check_files_with_identifiers(identifiers, all_files, check_type)
    assert unknown_files == ["apache_air-2.10.3.tar.gz",]